import React from "react";

const InfoPara = () => {
  return (
    <section className="flex info-para">
      <div className="pink-gradient-border glass info-content">
        <div className="flex ">
          <span className="mx-10">
            <svg
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              stroke="#ffffff"
              width={40}
            >
              <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
              <g
                id="SVGRepo_tracerCarrier"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></g>
              <g id="SVGRepo_iconCarrier">
                {" "}
                <path
                  d="M13.5 16.5854C13.5 17.4138 12.8284 18.0854 12 18.0854C11.1716 18.0854 10.5 17.4138 10.5 16.5854C10.5 15.7569 11.1716 15.0854 12 15.0854C12.8284 15.0854 13.5 15.7569 13.5 16.5854Z"
                  fill="#ffffff"
                ></path>{" "}
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
                  d="M6.33367 10C6.20971 9.64407 6.09518 9.27081 5.99836 8.88671C5.69532 7.68444 5.54485 6.29432 5.89748 4.97439C6.26228 3.60888 7.14664 2.39739 8.74323 1.59523C10.3398 0.793061 11.8397 0.806642 13.153 1.32902C14.4225 1.83396 15.448 2.78443 16.2317 3.7452C16.4302 3.98851 16.6166 4.23669 16.7907 4.48449C17.0806 4.89706 16.9784 5.45918 16.5823 5.7713C16.112 6.14195 15.4266 6.01135 15.0768 5.52533C14.9514 5.35112 14.8197 5.17831 14.6819 5.0094C14.0088 4.18414 13.2423 3.51693 12.4138 3.18741C11.6292 2.87533 10.7252 2.83767 9.64112 3.38234C8.55703 3.92702 8.04765 4.6748 7.82971 5.49059C7.5996 6.35195 7.6774 7.36518 7.93771 8.39788C8.07953 8.96054 8.26936 9.50489 8.47135 10H18C19.6569 10 21 11.3431 21 13V20C21 21.6569 19.6569 23 18 23H6C4.34315 23 3 21.6569 3 20V13C3 11.3431 4.34315 10 6 10H6.33367ZM19 13C19 12.4477 18.5523 12 18 12H6C5.44772 12 5 12.4477 5 13V20C5 20.5523 5.44772 21 6 21H18C18.5523 21 19 20.5523 19 20V13Z"
                  fill="#ffffff"
                ></path>{" "}
              </g>
            </svg>
          </span>
          <h2>Unlocking Financial Success </h2>
        </div>

        <div>
          <p>
            In today's dynamic economic landscape, managing your investments
            effectively is more crucial than ever. Whether you're a seasoned
            investor or just beginning your journey to financial independence,
            the importance of strategic investment management cannot be
            overstated. By actively overseeing your investment portfolio, you
            not only safeguard your financial future but also unlock a myriad of
            benefits that can propel you towards your wealth-building goals.
          </p>
          <p>
            First and foremost, effective investment management provides you
            with greater control and visibility over your financial assets.
            Moreover, disciplined investment management fosters long-term
            financial stability and security.
          </p>
          <p>
            In essence, investment management is not just about maximizing
            returns; it's about building a solid foundation for your financial
            future and achieving lasting prosperity. By prioritizing effective
            investment management strategies and leveraging the right tools and
            resources, you can navigate the complexities of the financial
            markets with clarity and conviction, unlocking new opportunities for
            wealth accumulation and financial success.
          </p>
        </div>
      </div>
    </section>
  );
};

export default InfoPara;
